                ...
                <div class="card-body">
                   ...
                   <form action="{{ route('admin.blog.update',$blog->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div><img src="{{ $blog->thumbnail_image ? url($blog->thumbnail_image) : '' }}" alt="old blog image" class="w_200"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="status" id="status" class="form-control">
                                    <option {{ $blog->status==1 ? 'selected' : ''}} value="1">{{ $websiteLang->where('id',140)->first()->custom_text }}</option>
                                    <option {{ $blog->status==0 ? 'selected' : ''}} value="0">{{ $websiteLang->where('id',141)->first()->custom_text }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="show_homepage" id="show_homepage" class="form-control">
                                    <option {{ $blog->show_homepage==0 ? 'selected' : ''}} value="0">{{ $websiteLang->where('id',124)->first()->custom_text }}</option>
                                    <option {{ $blog->show_homepage==1 ? 'selected' : ''}} value="1">{{ $websiteLang->where('id',123)->first()->custom_text }}</option>
                                </select>
                            </div>
                        </div>
