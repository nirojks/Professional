    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                   ...
                   <tr>
                       ...
                       <td>
                           @if ($order->payment_status==1)
                            ...
                            @else
                            <span class="badge badge-danger">{{ $websiteLang->where('id',81)->first()->custom_text }}</span>
                            @endif
                    </td>
                   </tr>
                </table>
                ...
                <a href="{{ route('admin.payment-accept',$order->id) }}" class="btn btn-success">{{ $websiteLang->where('id',497)->first()->custom_text }}</a>
                @endif
                <a href="{{ route('admin.order-delete',$order->id) }}" class="btn btn-danger">{{ $websiteLang->where('id',498)->first()->custom_text }}</a>
            </div>
        </div>
    </div>
@endsection
