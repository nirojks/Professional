...
class ListingCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $customMessages = [
            ...
            'icon_image.required' => $this->errorTexts->where('id',52)->first()->custom_text,
        ...
        $icon_image_name= 'listing-category-icon-'.date('Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
        $icon_image_path='uploads/custom-images/'.$icon_image_name;

        Image::make($icon_image)
            ->resize(600,null,function ($constraint) {
                $constraint->aspectRatio();
            })
            ->crop(45,45)
            ->save(public_path($icon_image_path));
        ...
        $category->icon_image=$icon_image_path;
    ...
    public function update(Request $request, ListingCategory $listingCategory)
    {
        ...
        if($request->icon_image){
            $old_icon_image=$listingCategory->icon_image;
            $icon_image=$request->icon_image;
            $extention=$icon_image->getClientOriginalExtension();
            $icon_image_name= 'listing-category-icon-'.date('Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
            $icon_image_path='uploads/custom-images/'.$icon_image_name;
            ...
            $listingCategory->icon_image=$icon_image_path;
            $listingCategory->save();
            if(File::exists(public_path($old_icon_image)))unlink(public_path($old_icon_image));
