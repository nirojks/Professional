...
class HomeController extends Controller
{
    ...
    public function blogDetails($slug){
        ...
        if($blog){
            ...
            $popularBlogs=Blog::where('id','!=',$blog->id)->orderBy('view','desc')->get()->take(10);
            ...
            $default_image=BannerImage::find(15);
            $setting=Setting::first();
            return view('user.blog.show',compact('blog','blogCategories','contact_us','popularBlogs','commentSetting','image','websiteLang','menus','default_image','setting'));
        }else{
            return back();
        }
    ...
    public function postDetails($slug){
        ...
        if($post){
            ...
            $default_image=BannerImage::find(15);
            $setting=Setting::first();
            return view('user.profile.post.show',compact('post','contact_us','image','websiteLang','menus','default_image','setting'));
        }else{
            return back();
        }
    ...
    public function blogComment(Request $request,$blogId){
        ...
        $notification=array(
            'messege'=>$this->notify->where('id',1)->first()->custom_text,
            'alert-type'=>'success'
        );
    ...
    public function pricingPlan(){
        ...
        return view('user.price-plan',compact('listingPackages','sections','seo_text','image','websiteLang','menus','currency'));
    }
    ...
    public function searchListingPage(Request $request){
        ...
        $listingAminities=ListingAminity::whereHas('listing',function($query) use ($request){
            ...
            if ($request->maplocation != null && $request->longlat != null) {
                $curl = curl_init();
                $radius = 5;
                ...
                $url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='.$request->search.'&radius='. $radius .'&key='.env('GOOGLE_MAP_API');
                ...
                foreach(json_decode($response)->results as $item) {
                    $comparePlace = $comparePlace .', ' . $item->name.', '.$item->vicinity; 
                }


                $query->where('title', 'LIKE', '%' . $comparePlace . '%')->orWhere('location_id', 'LIKE', '%' . $comparePlace . '%');
            }
    ...
    public function userProfile(Request $request){
        ...
        $websiteLang=$this->websiteLang;
        $menus=Navigation::all();
        return view('user.user-profile',compact('listings','days','listingCategories','aminities','locationsForSearch','seo_text','image','websiteLang','menus','default_image','user'));
